...
恢复包默认未额外加密，足以恢复整个部署。创建后应立即转移到加密离线存储，并按组织制度限制访问。企业恢复的账号托管份额在数据库中始终是按管理员账户公钥密封的密文；完整恢复仍必须同时保留数据库、运行密钥和管理员各自的账户解密能力。
...
工作台的企业恢复中心按“总览、准备恢复、恢复协助、历史记录”组织。服务器管理员先配置 2 至 6 位本地直授平台管理员：
...
启用前每位管理员都要先登录并设置自己的主密码。只配置两位即可恢复普通用户；若希望某位管理员本人忘记主密码时仍有另外两人协助，建议至少配置三位。企业恢复启用后不能撤到少于两位，最多六位。

首次准备在页面中分为三段：

1. 核对恢复管理员。列表中的“账号已就绪”只表示该管理员已经设置主密码并能够参与，不代表恢复设置已经完成。
2. 两位不同管理员分别明确确认设置。第一位管理员点击启用后，浏览器在 Crypto Worker 内生成企业恢复密钥，按当前管理员人数拆成 2-of-N 份额，并分别加密到每位管理员账号；服务端只保存密文。第二位管理员必须进入同一页面完成第二次确认，普通登录或解锁不会自动算作同意。
3. 第二位确认后首套设置立即自动启用。历史密码库由已解锁 owner 客户端后台补齐，不展示全库分数，也不要求管理员逐库点击。恢复协助会直接显示还差谁确认或系统正在处理。恢复密钥、管理员名单或管理员账号加密代次变化时，新设置会等所有现有密码库完成新覆盖后再切换，确保旧恢复能力被安全退役。

同事忘记主密码时，管理员在“恢复协助”按姓名、拼音或域账号搜索本人并发起协助，用户设置一次新主密码即可离开页面。第一位管理员确认后，浏览器只把本账号的一份恢复能力加密转交给其他管理员；第二位管理员确认后，其浏览器在 Crypto Worker 内短时组合两份能力，自动为用户当前公钥生成全部有效密码库信封，并原子启用新身份。用户之后可从任意浏览器重新登录。整个正常流程没有 ZIP、公开清单、三份文件、案件 JSON 或结果上传。

撤权、降级、恢复管理员名单变化、管理员或用户公钥换代、密码库换钥、过期和并发变化都会使交付失败关闭。若用户只有一个严格空的个人库，系统可以在第二次确认事务内删除旧空库并重新初始化“我的密码库”；只要存在活动条目、历史版本、目录或其他阻塞关联就拒绝替换。

旧离线向导、三份 `.mimashare`、案件 JSON 和结果上传协议只为兼容旧部署及安全审计保留，不出现在正常界面，也不能处理当前账号托管的恢复密钥。若未来确需改回离线保管，必须生成新离线恢复密钥，由密码库 owner 完成全库新覆盖后安全退役当前密钥。
