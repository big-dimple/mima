...
1. 第一位管理员在“准备恢复”点击启用。浏览器在 Crypto Worker 内生成企业恢复密钥，按当前管理员人数拆成 2-of-N 份额，并分别加密到每位管理员账号；服务端只保存密文。
2. 第二位管理员进入同一页面确认，首套设置立即启用。历史密码库由已解锁 owner 客户端后台补齐，不展示全库分数，也不要求管理员逐库点击。恢复密钥、管理员名单或管理员账号加密代次变化时，新设置会等所有现有密码库完成新覆盖后再切换，确保旧恢复能力被安全退役。
