...
2. 用户在浏览器中设置主密码并生成本地加密资料。
3. 如需平台管理员，由服务器管理员显式授予：
...
用户组名称或身份系统中的同名组不会自动获得平台管理员权限。平台管理员也不会自动取得任何密码库的解密密钥。
...
恢复包默认未额外加密，足以恢复整个部署。创建后应立即转移到加密离线存储，并按组织制度限制访问。企业恢复的 `.mimashare` 文件不属于服务器备份，必须由独立保管人离线保存。
...
企业恢复默认关闭，也不影响普通使用。启用它需要三名已准备好设备的本地直授平台管理员、两人审批流程，以及在隔离终端生成并分开保管的三份恢复材料。任意两份材料联合才能处理一次已批准请求。

工作台的企业恢复中心按“总览、准备恢复能力、待办审批、密码库保护、高级维护、我的恢复”分区。公开清单通过文件选择导入；页面只接收 `manifest.json`，不会接收 `.mimashare`。审批达到两名不同管理员后即可进入下一步，不需要第三人重复确认。

离线工具是 Compose 的 `tools` profile，不是常驻容器。先阅读工作台中的风险说明并完成组织制度设计，再启用该能力。不要把恢复材料上传到服务器、Git、聊天、工单或普通备份。恢复请求过期、取消、恢复密钥轮换或密码库密钥代际变化后必须重新发起，不要尝试复用旧结果。
