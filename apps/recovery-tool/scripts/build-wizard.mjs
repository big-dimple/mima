...
<main class="shell">
  ...
  <div class="mode-switch" role="tablist" aria-label="选择要做的事情">
    <button id="mode-setup" type="button" role="tab" aria-selected="true" aria-controls="setup-panel">首次准备</button>
    <button id="mode-case" type="button" role="tab" aria-selected="false" aria-controls="case-panel">处理恢复案件</button>
  </div>
    <section id="setup-panel" class="panel" role="tabpanel" aria-labelledby="mode-setup">
      <h2>首次准备企业恢复</h2>
    ...
    <section id="case-panel" class="panel" role="tabpanel" aria-labelledby="mode-case" hidden>
      <h2>处理一位同事的恢复案件</h2>
      <p>只有平台中已经出现具体恢复案件，并完成两位管理员确认后才使用这里。先在该案件点击“下载案件文件”，再选择下载得到的 JSON 文件和任意两份不同的恢复材料。</p>
      <form id="recovery-form">
        <div class="file-row"><label class="file-label" for="case-package">1. 从恢复案件下载的 JSON 文件</label><div class="file-picker"><input class="file-input" id="case-package" type="file" accept="application/json,.json" required><label class="file-button" for="case-package">选择文件</label><span class="file-name" id="case-package-name">尚未选择文件</span></div></div>
