...
const OPERATIONS: Record<string, OperationMetadata> = {
  ...
  'GET /api/v2/recovery/workspace': op('getRecoveryWorkspace', '读取企业恢复操作中心快照', '企业恢复', 'web'),
  'GET /api/v2/recovery/cases': op('listRecoveryCases', '读取恢复协助案件', '企业恢复', 'web'),
  'GET /api/v2/recovery/cases/{caseId}': op('getRecoveryCase', '读取恢复协助案件详情', '企业恢复', 'web'),
  'POST /api/v2/recovery/cases': op('createRecoveryCase', '管理员发起恢复协助', '企业恢复', 'web', true),
  'POST /api/v2/recovery/cases/{caseId}/target': op('finalizeRecoveryCaseTarget', '用户设置新的主密码', '企业恢复', 'web', true),
  'GET /api/v2/recovery/cases/{caseId}/approval-material': op('getRecoveryCaseApprovalMaterial', '读取管理员浏览器确认材料', '企业恢复', 'web'),
  'POST /api/v2/recovery/cases/{caseId}/approve': op('approveRecoveryCase', '管理员确认恢复协助', '企业恢复', 'web', true),
  'POST /api/v2/recovery/cases/{caseId}/cancel': op('cancelRecoveryCase', '取消恢复协助', '企业恢复', 'web', true),
  'GET /api/v2/recovery/cases/{caseId}/package': op('getRecoveryCasePackage', '下载一次恢复协助的离线处理包', '企业恢复', 'web'),
  'POST /api/v2/recovery/cases/{caseId}/transfers': op('uploadRecoveryCaseTransfer', '提交一次恢复协助的离线处理结果', '企业恢复', 'web', true),
  'GET /api/v2/recovery/cases/{caseId}/transfer': op('getRecoveryCaseTransfer', '领取一次恢复协助的处理结果', '企业恢复', 'web'),
  'POST /api/v2/recovery/key': op('registerRecoveryKey', '登记企业恢复公钥', '企业恢复', 'web', true),
  'POST /api/v2/recovery/custody': op('registerManagedRecoveryCustody', '准备管理员账号托管的企业恢复', '企业恢复', 'web', true),
  'POST /api/v2/recovery/keys/{keyId}/approve': op('approveRecoveryKey', '审批企业恢复公钥', '企业恢复', 'web', true),
  'POST /api/v2/recovery/keys/{keyId}/cancel': op('cancelRecoveryKey', '取消未启用的企业恢复公钥', '企业恢复', 'web', true),
  'GET /api/v2/recovery/keys/{keyId}/custody/share': op('getManagedRecoveryCustodyShare', '读取当前管理员的加密恢复份额', '企业恢复', 'web'),
  ...
  'POST /api/v2/recovery/requests/{requestId}/cancel': op('cancelRecoveryRequest', '取消企业恢复请求', '企业恢复', 'web', true),
