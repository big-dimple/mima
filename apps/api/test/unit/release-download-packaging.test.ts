...
describe('release download packaging', () => {
  it('uses the repository packager and current schema head in the public image build', () => {
    ...
    expect(dockerfile).toContain('mima.schema-head="0023_enterprise_recovery_integrity"');
  ...
  it('creates deterministic extension and recovery archives without an operating-system zip package', () => {
    ...
    const first = runPackager(extensionDirectory, recoveryToolFile, usageFile, outputDirectory);
    ...
    expect(firstRecovery.includes(Buffer.from('mima-recovery-tool.mjs'))).toBe(true);
    expect(firstRecovery.includes(Buffer.from('OFFLINE-USAGE.txt'))).toBe(true);
    ...
    const second = runPackager(extensionDirectory, recoveryToolFile, usageFile, outputDirectory);
