...
忘记主密码时，用户直接联系公司管理员。管理员在企业恢复中心发起协助，用户设置一次新主密码即可离开页面；第二位管理员确认后系统自动启用新主密码并注销旧登录，用户可从任意浏览器重新登录，不需要返回设置新主密码时使用的浏览器。需要恢复的原有访问由管理员继续处理，用户不需要操作离线工具或文件。系统只恢复用户原本仍然有效的密码库权限；不会找回旧主密码、不会让管理员登录用户账号，也不会给任何人新增权限。

恢复案件通过姓名、拼音或域账号服务端搜索同事，不加载全公司下拉名单，也不会默认选中第一人。“准备恢复”始终列出当前三名恢复管理员的姓名、登录账号和准备状态，并明确每次恢复只需其中两位确认。离线向导放在断网电脑使用，是为了让恢复材料在生成和联合处理时始终不接触服务器或网络。
