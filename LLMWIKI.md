...
- 数据库迁移头：`0023_enterprise_recovery_integrity`
...
- 默认关闭，不阻塞普通使用。
- 三名管理员和三名离线材料保管人是两种职责，不是同一概念。
- 管理员候选只来自显式本地直授的活动 OIDC `platform-admin`，且必须有当前加密资料和活动设备。
- 每次恢复需要两名不同管理员审批；目标用户不能自批。
- 恢复公钥、逐库恢复和账户加密重置都只接受精确两名不同管理员审批。状态转换必须在事务内锁定并重新核对；第三份审批、过期请求、已取消请求、恢复密钥轮换或密码库 epoch 变化都必须拒绝。
- 离线恢复密钥拆成三份 `.mimashare`，任意两份只能在隔离终端联合使用。
- 服务器只保存恢复公钥、指纹、ceremony digest、代际、审批和 opaque evidence，不保存 share。
- 逐库恢复请求创建时必须绑定 active recovery key 和密码库 `keyEpoch`；完成信封使用创建时已经验证的签名者 user/key version/public key 快照，不能改用当前 profile。升级前缺少该快照的活动恢复信封视为未覆盖，需要已解锁 owner 重新签名。
- 幂等键同时绑定命令名和请求摘要；同一键重试相同请求可以复用结果，同一键提交不同内容必须拒绝。
- 恢复不能新增权限，不能恢复旧主密码；团队库只恢复给仍有合法权限的用户，个人库只恢复给原所有者。
- 只要仍有 owner 能解锁，应直接重新分发，不走企业恢复。
- 首次激活前，已解锁 owner 必须为明确纳入保护的密码库写入恢复公钥信封；服务端不能自行生成。
