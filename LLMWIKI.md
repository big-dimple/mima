...
- 数据库迁移头：`0024_enterprise_recovery_cases`
...
- 有 owner 能解锁时，owner 客户端自动满足对应子任务；没有 owner 能解锁时，一个离线处理包覆盖案件内全部未完成子任务。离线结果上传后，用户可从任意浏览器用新主密码解锁，由当前活动登录自动领取、校验和完成，不依赖最初设置新主密码的浏览器。
- 离线恢复密钥拆成三份 `.mimashare`，放在三个独立位置；任意两份只能在隔离终端联合使用，不存在产品内“材料保管人”角色。
- ZIP 顶层必须提供可直接双击的“打开企业恢复向导.html”。生成材料无需填写标识、目录或文件名；处理案件只选择一个处理包和两份不同材料。Node.js 命令行入口只能放在 `advanced/` 作为受控环境兜底。
...
- 首次启用或轮换恢复公钥时，已解锁 owner 客户端在后台为明确纳入保护的密码库写入恢复信封；服务端不能自行生成。整库完成后只刷新一次，网络失败在下次事件、解锁或重连时自动重试。
