...
- `apps/api/src/routes/e2ee-recovery-cases.ts`：管理员发起、用户准备、双人确认、管理员密文 relay、浏览器自动完成和领取前过滤的恢复案件状态机。
...
- `apps/recovery-tool/src/cli.ts`：旧离线恢复材料协议的内部兼容工具，不进入当前正常产品流程。
...
- 默认关闭，不阻塞普通使用。
- 管理员候选只来自显式本地直授、已经设置主密码的活动 `platform-admin`；可配置 2 至 6 位，企业恢复启用后不能撤到少于两位。平台角色本身不授予任何密码库内容权限。
- 第一位管理员启用时，Crypto Worker 生成企业 X25519 恢复密钥并按当前管理员人数拆成 2-of-N 份额，每份只用对应管理员账户加密公钥密封。服务器只保存恢复公钥、指纹、代际、管理员公钥代次、密文份额、审批和 opaque evidence，不保存恢复私钥或明文份额。
- 恢复公钥和每次恢复案件都只接受精确两名不同管理员确认，目标用户不能确认自己的案件。状态转换必须在事务内锁定并重新核对；第三份审批、过期或取消、管理员集合变化、恢复密钥轮换、密码库 epoch 变化和收件人公钥代次变化都必须拒绝。
- 忘记主密码由管理员先发起案件，用户只设置一次新主密码即可离开。第一位管理员确认时，Crypto Worker 打开本账号份额并分别加密 relay 给其余管理员；第二位管理员用自己的份额和第一人的 relay 在 Crypto Worker 内短时重建恢复私钥，自动生成只面向用户当前账户公钥的密码库信封后立即销毁临时密钥。
- 第二位管理员提交时，API 在同一事务内复查管理员集合、公钥代次、案件、账户重置、实时权限、密码库 epoch、完整信封集合和任务 CAS，再自动启用新加密代次并注销旧登录。用户可从任意浏览器重新登录，不依赖最初设置新主密码的浏览器。
- 案件一次绑定用户当时仍然有效的全部原有密码库权限。普通界面没有申请自助、处理包、设备选择、ZIP、公开清单、案件 JSON、结果上传或密码学字段。
- “准备恢复”始终列出当前 2 至 6 位恢复管理员的姓名、登录账号和准备状态，明确每次只需其中任意两人；“账号已就绪”只表示账号具备参与条件，不等于已经确认设置。首次设置和轮换必须由两位不同管理员分别明确确认，普通登录或解锁不得静默计数。“恢复协助”复用服务端姓名/域账号搜索，支持通过拼音域账号查找，不加载全公司下拉名单、不默认选中第一人；未启用时必须显示还差谁确认、账号缺什么准备或系统正在处理，并提供直达“准备恢复”的操作。
- 首次设置由两名管理员确认后立即启用，历史密码库在 owner 下次解锁或重连时后台补齐，不向管理员展示全库覆盖分数。恢复密钥、管理员集合或管理员账号加密代次变化时，新 key 必须先覆盖全部现有密码库 active epoch，才能退役旧 key。
- 若目标用户唯一的个人库不存在活动条目、历史版本、目录或其他阻塞关联，第二位管理员确认事务可删除旧空库并重新初始化带活动恢复信封的“我的密码库”；空库见证在事务内重算，任何内容或竞态都失败关闭。
- 案件子任务绑定 active recovery key、密码库 `keyEpoch`、目标与管理员当前公钥代次、能力和创建时已验证的签名者快照。完成前再次计算实时授权；撤权、降级、旧公钥、旧 epoch、旧恢复 key 和过期任务不得交付。
- 幂等键同时绑定命令名和请求摘要；同一键重试相同请求可以复用结果，同一键提交不同内容必须拒绝。
- 恢复不能新增权限，不能恢复旧主密码；团队库只恢复给仍有合法权限的用户，个人库只恢复给原所有者。
- 旧离线向导、三份 `.mimashare`、案件 JSON 和结果上传协议只保留内部兼容。它们不能处理当前账号托管 key；切回离线模式必须生成新 key，由 owner 完成全库覆盖后安全退役当前 key。
...
- 企业恢复界面只说明“管理员发起、用户设置一次新主密码、两人确认、系统恢复原有权限”。用户设置后可以离开页面；第二位管理员确认会在其浏览器自动完成并注销旧登录，之后用户可从任意浏览器重新登录。不得要求用户选择浏览器或设备，不得把公钥、代次、信封、摘要、处理包等工程语言展示给普通用户。
